classdef LossTest < matlab.unittest.TestCase
  ...
  methods (Test)
    function zerooneTest(testCase)
      ...
      testCase.verifyError(@() zeroone([]), '');
      testCase.verifyError(@() zeroone(1), '');
      testCase.verifyError(@() zeroone([1 1]), '');
    end
    ...
    function hingeTest(testCase)
      ...
      testCase.verifyError(@() hinge([]), '');
      testCase.verifyError(@() hinge(1), '');
      testCase.verifyError(@() hinge([1 1]), '');
    end

    function lossTest(testCase)
      P = [0.3 0.5 0.2; 0.9 0.05 0.05; 0.1 0.2 0.8]';
      L = [0 1 2; 1 0 1; 2 1 0];
      [actMinLoss, actIdx, actPredProbs, actLosses] = loss(P, L);

      expMinLoss = [0.5 0.15 0.4];
      expIdx = [2 1 3];
      expPredProbs = [0.5 0.9 0.8];
      expLosses = [0.9 0.5 1.1; 0.15 0.95 1.85; 1.8 0.9 0.4]';
      ...
      testCase.verifyError(@() loss([1 1]), '');
      testCase.verifyError(@() loss([0.4; 0.6], L), '');
    end
