classdef CovTest < matlab.unittest.TestCase
  ...
  methods (Test)
    function sqexpTest(testCase)
      ...
      [actK1, ~] = sqexpard(Xm, Xn, [sigma; l; l], [1; 2]);

      testCase.verifyEqual(actK, actK1, 'AbsTol', 1e-2);

      [actK, actdK] = sqexp(Xm, 'diag', [sigma; l], [1; 2]);
      expK = sigma^2 * exp(-zeros(m, n)/2);
      expdK = zeros(m, n, 2);
      expdK(:, :, 1) = 2 * sigma * exp(-zeros(m, n)/2);

      testCase.verifyEqual(actK, expK, 'AbsTol', 1e-4);
      testCase.verifyEqual(actdK, expdK, 'AbsTol', 1e-4);

      [actK, actdK] = sqexp(Xm, Xm, [sigma; l], [1; 2]);
      testCase.verifyTrue(issymmetric(actK));
      testCase.verifyEqual(diag(actK), sigma^2 * ones(m, 1), 'AbsTol', 1e-4);
      testCase.verifyEqual(diag(actdK(:, :, 1)), 2 * sigma * exp(-zeros(m, 1)/2), 'AbsTol', 1e-4);
    ...
    function sqexpardTest(testCase)
      ...
      [actK, actdK] = sqexpard(Xm, Xn, [sigma; l], [1; 2]);
      [expK, expdK] = CovTest.sqexpNaive(Xm, Xn, [sigma; l], [1; 2], 'ard');
    ...
    function [Kmn, dKmn] = sqexpNaive(Xm, Xn, theta, j, varargin)
      ...
      for p = 1:m
        for q = 1:n
          ...
          for k = 1:length(j)
            if k == 1
              dKmn(p, q, k) = 2 * sigma * exp(-Kmn(p, q) / 2);
            else
              if nargin > 4 && strcmpi(varargin{1}, 'ard')
                dKmn(p, q, k) = sigma^2 * exp(-Kmn(p, q) / 2) * (sqdistk(Xm(p, k-1), Xn(q, k-1))/l(k-1).^3);
              else
                dKmn(p, q, k) = sigma^2 * exp(-Kmn(p, q) / 2) * (Kmn(p, q) / l);
              end
            end
          end

          Kmn(p, q) = sigma^2 * exp(-Kmn(p, q)/2);
        end
      end
    end
  end
